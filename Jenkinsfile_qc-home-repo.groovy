pipeline {
    ...
    parameters {
        booleanParam(defaultValue: true, description: 'Force full deployment of qc-home-repo (ignores changes).', name: 'FORCE_FULL_DEPLOY')
    }
    ...
    environment {
        ...
        // 배포 필요 여부를 저장할 플래그
        SHOULD_DEPLOY_QC_HOME_REPO = 'false'
    }
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                // !!! 중요: qc-home-repo의 실제 GitHub Repository URL로 변경 필요 !!!
                git branch: 'deployNAS', credentialsId: 'github-token-333', url: 'https://github.com/jigubangbang/qc-home-repo.git' // qc-home-repo의 가정한 레포지토리 URL
            }
        }
            ...
            steps {
                script {
                    ...
                    sh """
                        ...
                        echo "Cleaning up old Docker images for qc-home-repo..."
                        docker rmi \$(docker images ${env.DOCKER_REGISTRY}/qc-home-repo --format "{{.Repository}}:{{.Tag}}" | grep -v ":${env.IMAGE_TAG}\\|:latest" | head -5) 2>/dev/null || true
            ...
            steps {
                script {
                    ...
                    if (params.FORCE_FULL_DEPLOY || env.BUILD_NUMBER == '1') {
                        env.SHOULD_DEPLOY_QC_HOME_REPO = 'true'
                        echo "Force full deployment or first build - deploying qc-home-repo."
                    } else {
                        // 변경 사항 감지 (단일 레포이므로 단순히 변경이 있으면 배포)
                        def rawChanges = sh(returnStdout: true, script: 'git diff --name-only HEAD HEAD^1 || true').trim()
                        if (!rawChanges.isEmpty()) {
                            env.SHOULD_DEPLOY_QC_HOME_REPO = 'true'
                            echo "Detected changes in qc-home-repo repository. Deploying."
                        } else {
                            env.SHOULD_DEPLOY_QC_HOME_REPO = 'false'
                            echo "No changes detected in qc-home-repo. Skipping deployment."
        ...
        stage('Deploy QC-Home-Repo Bundle') {
            when {
                expression { return env.SHOULD_DEPLOY_QC_HOME_REPO == 'true' }
            }
            steps {
                script {
                    def serviceName = 'qc-home-repo'
    ...
    post {
        ...
        failure {
            echo "❌ CI/CD Pipeline failed for qc-home-repo."
        }
        success {
            echo "✅ CI/CD Pipeline finished successfully for qc-home-repo."
        }
    }
}